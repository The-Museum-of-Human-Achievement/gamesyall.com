...
# Default values
DEFAULT_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTamZKS-_znOYuDxfHNZMkSK5dXx8_aBFiN2IyzD-OVP50EMQAPfGPUgTy4AmYyYRPrMDHmEyG2445w/pub?gid=1470403034&single=true&output=csv"
DEFAULT_GAMES_DIR = "../games_yall_site/_games"
DEFAULT_EVENTS_DIR = "../games_yall_site/_events"
DEFAULT_LOCAL_CSV = "../_data/games_data.csv"  # Default name for local CSV file
...
def process_games_data(csv_source, games_dir, events_dir=None, is_url=False, debug=False, site_root=None):
    ...
    try:
        ...
        for row in rows:
            ...
            # Get month name for event organization
            ...
            developer = row.get(developer_field[0] if developer_field else '', '')
            ...
            description = row.get(description_field[0] if description_field else '', '')
...
if __name__ == "__main__":
    ...
    if args.csv is None:
        if args.local:
            # If --local flag is set but no CSV specified, use default local CSV
            args.csv = DEFAULT_LOCAL_CSV
            is_url = False
            print(f"Using default local CSV: {args.csv}")
        else:
            # Otherwise use the default URL
            args.csv = DEFAULT_CSV_URL
            is_url = True
            print(f"Using default CSV URL: {args.csv}")
    else:
