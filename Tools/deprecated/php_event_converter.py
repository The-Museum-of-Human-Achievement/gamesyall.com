...
def extract_date_from_breadcrumbs(soup):
    ...
    breadcrumbs = soup.find('div', class_='row', style=lambda value: value and 'margin-bottom: 0px' in value)
    if breadcrumbs:
        breadcrumb_div = breadcrumbs.find('div', class_='col-4')
        if breadcrumb_div:
            breadcrumb_text = breadcrumb_div.get_text()
...
def extract_date_from_filename(filename):
    ...
    # Look for month-year pattern in filename (e.g., "event-april-2022.html" or "event-april-2022.php")
    pattern = r"event-([a-z]+)-(\d{4})\.(html|php)"
    match = re.search(pattern, filename.lower())
    if match:
        month_str, year, _ = match.groups()
        if month_str in months:
            month_num = months[month_str]
            return f"{year}-{month_num:02d}-01"
    
    # Look for year first in filename (e.g., "event-2022-11.html" or "event-2022-11.php")
    pattern2 = r"event-(\d{4})-(\d{2})\.(html|php)"
    match = re.search(pattern2, filename.lower())
    if match:
        year, month, _ = match.groups()
        return f"{year}-{month}-01"
    
    # Look for specific month names in filename (e.g., "event-november-2022.html" or "event-november-2022.php")
...
def extract_meta_vars_from_php(php_content):
    """
    Extract metadata variables from PHP content like $headTitle, $headDescription, $headImage
    """
    meta = {}
    
    # Extract $headTitle
    title_match = re.search(r'\$headTitle\s*=\s*"([^"]+)"', php_content)
    if title_match:
        meta['title'] = title_match.group(1)
    
    # Extract $headDescription
    desc_match = re.search(r'\$headDescription\s*=\s*"([^"]+)"', php_content)
    if desc_match:
        meta['description'] = desc_match.group(1)
    
    # Extract $headImage
    img_match = re.search(r'\$headImage\s*=\s*"([^"]+)"', php_content)
    if img_match:
        meta['image'] = img_match.group(1)
    
    return meta

def extract_php_date_formats(php_content):
    """
    Extract date from common PHP date format strings
    """
    # Try to find date formats like "March 8th, 2025" in PHP content
    date_pattern = r"([A-Za-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?,\s+(\d{4})"
    match = re.search(date_pattern, php_content)
    if match:
        month_str, day, year = match.groups()
        month_names = {
            'january': 1, 'jan': 1,
...
def convert_files_to_md(input_folder, output_folder):
    """
    Convert HTML/PHP files in a folder to Jekyll markdown files for an Events collection.
    
    Args:
        input_folder (str): Path to folder containing HTML/PHP files
    ...
    # Process each HTML/PHP file in the folder
    for filename in os.listdir(input_folder):
        if filename.endswith(('.html', '.php')):
            ...
            # Extract metadata from PHP files
            php_meta = {}
            if filename.endswith('.php'):
                php_meta = extract_meta_vars_from_php(file_content)
                # Try to extract date from PHP variable declarations
                php_date = extract_php_date_formats(file_content)
                
                # Remove PHP code blocks to make BeautifulSoup parsing easier
                # This regex removes PHP code between <?php and ?> tags
                file_content = re.sub(r'<\?php.*?\?>', '', file_content, flags=re.DOTALL)
            ...
            # Extract metadata
            title = None
            if php_meta and 'title' in php_meta:
                title = php_meta['title']
            else:
            ...
            # For PHP files, first try the extracted PHP date format
            if filename.endswith('.php') and php_date:
                formatted_date = php_date
            
            # 1. Try to extract from page title
            if not formatted_date and title:
                formatted_date = extract_date_from_title(title)
                
            # 2. Try to extract from h1/h2/h3/h4 tags (often contain event dates)
            ...
            if not formatted_date:
                formatted_date = extract_date_from_text(file_content)
            
            # 7. Last resort: default date with the correct year if possible
            if not formatted_date:
                # Try to at least get the year right
                year_match = re.search(r"\b(20\d{2})\b", file_content)
                if year_match:
                    year = year_match.group(1)
            ...
            event_desc = ""
            desc_div = soup.find('div', class_='col-4', style=lambda value: value and 'justify-content: left' in value)
            if desc_div:
            ...
            # Extract event time
            event_time = ""
            time_tag = soup.find('h4')
            if time_tag:
                time_text = time_tag.get_text()
                time_match = re.search(r'\d+-\d+(?:am|pm)', time_text, re.IGNORECASE)
                if time_match:
                    event_time = time_match.group(0)
            
            # Extract event location
            event_location = ""
            location_tag = soup.find('a', href=lambda href: href and ('maps' in href or 'goo.gl' in href))
            if location_tag:
                event_location = location_tag.get_text().strip()
            
            # Extract event image
            event_img = ""
            if php_meta and 'image' in php_meta:
                event_img = php_meta['image']
            else:
            ...
            for card in game_cards:
                ...
                # Game description
                desc_tag = card.find_all('div')[1].find('p') if len(card.find_all('div')) > 1 else None
            ...
            # Add time if found
            if event_time:
                yaml_front_matter += f"time: \"{event_time}\"\n"
            
            # Add location if found
            if event_location:
                yaml_front_matter += f"location: \"{event_location}\"\n"
            
            # Add featured image
            yaml_front_matter += f"featured_image: \"{event_img}\"\n"
            
            # Add description if we have one from PHP meta
            if php_meta and 'description' in php_meta:
                yaml_front_matter += f"description: \"{php_meta['description']}\"\n"
            ...
            if games:
                ...
                for game in games:
                    ...
                    if 'developer_bio' in game and game['developer_bio']:
                        bio_lines = game['developer_bio'].split('\n')
                        yaml_front_matter += f"    developer_bio: |\n"
                        for line in bio_lines:
                            yaml_front_matter += f"      {line}\n"
                    
                    if 'social_links' in game and game['social_links']:
            ...
            # Extract main event description
            main_content_div = soup.find('div', class_='col-10')
            if main_content_div:
                content_html = str(main_content_div)
                content_sections.append(converter.handle(content_html))
            
            # Also try the standard content location
            content_div = soup.find('div', class_='col-4', style=lambda value: value and 'justify-content: left' in value)
            if content_div:
                content_html = str(content_div)
                content_sections.append(converter.handle(content_html))
            
            # Combine all content sections and remove duplicate content
            ...
            # Create output filename - strip extension and use date as prefix
            base_filename = os.path.splitext(filename)[0]
            output_filename = f"{formatted_date}-{base_filename}.md"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Convert HTML/PHP files to Jekyll markdown for an Events collection')
    parser.add_argument('--input', default='html_files', help='Input folder containing HTML/PHP files')
    parser.add_argument('--output', default='_events', help='Output folder for markdown files')
    
    args = parser.parse_args()
    
    convert_files_to_md(args.input, args.output)
