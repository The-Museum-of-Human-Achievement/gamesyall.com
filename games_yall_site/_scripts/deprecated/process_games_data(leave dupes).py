#!/usr/bin/env python3
# process_games_data_corrected.py
#
# Updated to work with the ACTUAL Google Form CSV format
...
def process_games_data(csv_source, games_dir, events_dir=None, is_url=False, debug=False, site_root=None):
    """
    Process games from Google Form CSV without overwriting existing files
    Updated to work with the actual CSV field names from the form
    """
    ...
    try:
        ...
        for row in rows:
            ...
            if pronunciation:
                game_data['pronunciation'] = sanitize_string(pronunciation)
            
            if credits:
                game_data['credits'] = sanitize_string(credits)
            
            if call_to_action_text:
                game_data['call_to_action_text'] = sanitize_string(call_to_action_text)
            
            if info_sheet_description and info_sheet_description != description:
                game_data['info_sheet_description'] = sanitize_string(info_sheet_description)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process games from Google Form CSV without overwriting existing files')
    ...
    try:
        print("\n=== Starting Games Y'all Data Processing (Google Form Format) ===")
        ...
        print("Note: Works with Google Form CSV format")
        print("=" * 60 + "\n")
        ...
        if is_url:
            ...
            print("   python process_games_data_corrected.py --local --csv path/to/downloaded/file.csv\n")
        if args.debug:
            import traceback
            traceback.print_exc()
        sys.exit(1)