...
#
# This script processes game data from a CSV file and creates/updates game and event files.
# It automatically downloads promotional images from Google Drive and stores them locally
# in the format: /images/Archived-img/[Month-Year]/[game-title].jpg
#
# Required packages:
...
DEFAULT_LOCAL_CSV = "games_data.csv"  # Default name for local CSV file

# Image storage settings
# Images are stored in: /images/Archived-img/[Month-Year]/[game-title].[ext]
# Example: /images/Archived-img/May-2025/v-shift.jpg
...
def process_games_data(csv_source, games_dir, events_dir=None, is_url=False, debug=False, site_root=None):
    """
    Process games from CSV without overwriting existing files
    
    Args:
        csv_source (str): Path to the CSV file or URL
        games_dir (str): Directory where game files are stored
        events_dir (str): Optional directory for event files
        is_url (bool): Whether csv_source is a URL
        debug (bool): Whether to print detailed debug information
        site_root (str): Root directory of the site (for image paths)
    """
    ...
    if os.path.exists(games_dir):
        for filename in os.listdir(games_dir):
            if filename.endswith('.md') or filename.endswith('.html'):
                existing_games.add(os.path.splitext(filename)[0])
    
    if debug:
        print(f"Found {len(existing_games)} existing game files")
    ...
    # Track new games by month/year for event creation
    games_by_month = {}
    new_games_count = 0
    images_downloaded = 0
    ...
    try:
        ...
        for row in rows:
            # Skip empty rows
            game_title_field = [f for f in ['What is the title of your project', 'Game Title', 'title'] if f in row and row.get(f)]
            if not game_title_field:
                if debug:
                    print("Skipping row with empty Game Title")
                    print(f"Available fields: {list(row.keys())}")
                continue
            
            game_title = row.get(game_title_field[0], '').strip()
            ...
            if timestamp_field not in row or not row.get(timestamp_field):
                if debug:
            ...
            event_month = None
            if event_month_field in row and row.get(event_month_field):
            ...
            # Create game slug
            game_slug = slugify(f"{date_str}-{game_title}")
            
            # Skip if game already exists (to preserve manual edits)
            if game_slug in existing_games:
                if debug:
                    print(f"Skipping existing game: {game_slug}")
                continue
            
            # Get developer field - use the "Developer Name" from form or parse from credits field
            developer_field = [f for f in ['Developer Name', 'Developer/Studio/Publisher', 'developer'] if f in row and row.get(f)]
            developer = row.get(developer_field[0] if developer_field else '', '').strip()
            
            # If no developer field found, try to extract from credits field
            if not developer:
                credits_field = 'What are the names, name pronunciations, and pronouns of the developers you want credited during announcements?'
                if credits_field in row and row.get(credits_field):
                    # Extract just the names (before the pronouns/pronunciation info)
                    credits_text = row.get(credits_field, '').strip()
                    # Try to extract the developer name - take first part before parentheses or commas
                    developer_parts = credits_text.split('(')[0].split(',')[0].strip()
                    if developer_parts:
                        developer = developer_parts
            
            # Get description field
            description_field = [f for f in ['Game Description for Website', 'Game Description for Info Sheet', 'Game Info', 'description'] if f in row and row.get(f)]
            description = row.get(description_field[0] if description_field else '', '').strip()
            
            # Get developer bio field
            bio_field = [f for f in ['Developer Bio for Info Sheet', 'Developer Information', 'developer_bio'] if f in row and row.get(f)]
            developer_bio = row.get(bio_field[0] if bio_field else '', '').strip()
            
            # Get website/call to action field
            website_field = [f for f in ['Call to action link', 'Website', 'website'] if f in row and row.get(f)]
            website = row.get(website_field[0] if website_field else '', '').strip()
            ...
            image_field = [f for f in ['Promotional Image', 'Image URL', 'image'] if f in row and row.get(f)]
            if image_field and row.get(image_field[0]):
                image_url = row.get(image_field[0])
                ...
                if local_image_path:
                    game_data['image'] = local_image_path
                    if was_downloaded:
                        images_downloaded += 1
                elif debug:
            ...
            featured_field = [f for f in ['Featured', 'featured'] if f in row and row.get(f)]
            if featured_field and row.get(featured_field[0]):
                is_featured = row.get(featured_field[0], '').lower()
                if is_featured in ('yes', 'true', '1'):
                    game_data['featured'] = True
            
            # Extract social media links
            social_field = [f for f in ['Additional Links or Social Media Handles', 'Social Media', 'social_links'] if f in row and row.get(f)]
            if social_field and row.get(social_field[0]):
                social_links = extract_social_links(row.get(social_field[0]))
                if social_links:
                    game_data['social_links'] = social_links
            
            # Write game file
            output_file = os.path.join(games_dir, f"{game_slug}.md")
            with open(output_file, 'w', encoding='utf-8') as out:
                out.write('---\n')
                yaml.dump(game_data, out, default_flow_style=False, allow_unicode=True)
                out.write('---\n\n')
                
                # Add content if we have a description
                if game_data.get('description'):
                    out.write(game_data['description'])
            
            print(f"Created new game file: {output_file}")
            new_games_count += 1
            ...
            if events_dir and month_name:
                ...
                games_by_month[month_year].append(game_slug)
    ...
    if events_dir and games_by_month:
        for month_year, game_slugs in games_by_month.items():
            ...
            event_pattern = f"{year}-{month_name.lower()}-*.md"
            matching_events = []
            if os.path.exists(events_dir):
                for filename in os.listdir(events_dir):
                    if re.match(f"{year}-\d{{2}}-.*-event-{month_name.lower()}-{year}\.(md|html)$", filename):
                        matching_events.append(filename)
            
            if matching_events:
                # Event file exists - update it by adding new games
                ...
                if front_matter_match:
                    try:
                        ...
                        existing_slugs = front_matter.get('game_slugs', [])
                        ...
                        # Update front matter
                        front_matter['game_slugs'] = existing_slugs
                        ...
                        print(f"Updated event file with new games: {event_file}")
                    except Exception as e:
    ...
    if new_games_count > 0:
        print(f"Successfully processed {new_games_count} new games across {len(games_by_month)} months")
        if images_downloaded > 0:
            print(f"Downloaded {images_downloaded} promotional images from Google Drive")
    else:
        print("No new games were processed. This could mean either:")
        print("1. All games in the CSV already exist in the games directory")
        print("2. There were issues with parsing the CSV data")
        if debug:
            print("Check debug output above for more details")
    
    return new_games_count, images_downloaded
...
def suggest_download_instructions():
    """Print instructions for manually downloading the CSV"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process games without overwriting existing files')
    ...
    parser.add_argument('--site-root', 
                        default=None,
                        help='Root directory of the site (for image storage paths)')
    ...
    try:
        print("\n=== Starting Games Y'all Data Processing ===")
        ...
        print("Note: Google Drive images will be downloaded locally")
        print("=" * 40 + "\n")
        
        process_games_data(args.csv, args.games, args.events, is_url, args.debug, args.site_root)
